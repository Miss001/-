...
# 物理架构
- 一个database cluster的基础目录(base directory)为：DN_DATA
- 初始化完成后，会有两个默认的表空间
  一个是pg_default：如果用户建表时没有指定表空间，所有新建的表都会放在pg_default中,对应目录是 $DN_DATA/base
  一个是pg_global：存放的是整个实例数据库所共享的系统表。对应目录是 $DN_DATA/global
| 文件名          | 描述                                                         |
| --------------- | ------------------------------------------------------------ |
| base            | 表和索引文件存放目录                                         |
| global          | 存储全局系统表的数据                                         |
