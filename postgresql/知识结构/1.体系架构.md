...
- 叶子节点  
  树的最底层节点，存储实际的键值及其对应的数据 TID（Tuple ID），可以用于直接访问 Heap Page中的实际数据
数据页结构
- header
  存储LSN号、校验位等元数据信息，占用24Bytes
- pd_linp
  存储指向实际数据的指针
- free space
  空闲空间
...
- special space
  页预留的位置
![image](https://github.com/user-attachments/assets/db4f33e9-57e1-435d-af0e-5bb8369c4217)
